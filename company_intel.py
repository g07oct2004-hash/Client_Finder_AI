...
# # Final Output File (Processed Data)
# FINAL_OUTPUT_FILE = "company_intel/Final_Company_Data_Tavily_Only.json"

# # Raw Log File (Will be deleted at start of every run)
# RAW_DEBUG_FILE = "raw_search_logs_tavily.json"
...
#     """
#     Saves the raw Tavily response and the AI Direct Answer into a JSON file.
#     """
#     new_entry = {
#         "company": company,
#         "source": source,
#         "timestamp": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
#         "tavily_direct_answer": direct_answer,  # The AI answer from Tavily
#         "raw_search_context": raw_text          # The search results
#     }
...
#     # 3. Write back to file in JSON format
#     try:
...
# def save_json(data):
#     """
#     Saves the final processed data to JSON.
#     """
#     try:
#         with open(FINAL_OUTPUT_FILE, "w", encoding="utf-8") as f:
#             json.dump(data, f, indent=4, ensure_ascii=False)
...
# # ==========================================
# #  3. TAVILY SEARCH (WITH DIRECT ANSWER)
# # ==========================================

# def search_via_tavily(company):
#     """
#     Searches using Tavily API with 'include_answer=True' to get a direct summary.
#     """
#     current_year = datetime.datetime.now().year
#     prev_year = current_year - 1
    
#     # Specific Query
#     query = f"What is the total annual revenue of {company} in {prev_year} and total employee count in {current_year}?"
...
#             # Executing Search with DIRECT ANSWER enabled
#             response = client.search(
#                 query=query, 
#                 search_depth="advanced", 
#                 max_results=2, 
...
#             # 1. Extract Direct Answer (The AI Summary)
#             direct_answer = response.get("answer", "No direct answer generated.")
            
#             # 2. Extract Search Results (The Context)
#             context_text = ""
#             for result in response.get("results", []):
#                 context_text += f"Title: {result['title']}\nContent: {result['content']}\n----------\n"
...
# def analyze_with_groq(company, raw_data, direct_answer):
#     """
#     Uses Groq API to extract data. We feed it BOTH the raw context AND the direct answer.
#     """
#     max_retries = max(1, get_groq_count())
    
#     # Combine the Direct Answer and the Search Results for better context
#     full_context = f"TAVILY AI SUMMARY: {direct_answer}\n\nSEARCH RESULTS:\n{raw_data}"
...
# def main():
#     # --- AUTO DELETE OLD LOGS Logic ---
#     if os.path.exists(RAW_DEBUG_FILE):
#         try:
#             os.remove(RAW_DEBUG_FILE)
#             print(f"🗑️  Deleted old log file: {RAW_DEBUG_FILE}")
#         except Exception as e:
#             print(f"⚠️ Could not delete old log file: {e}")
#     # ----------------------------------
...
#         # --- 1. SEARCH (TAVILY) ---
#         raw_data, direct_answer = search_via_tavily(company)
        
#         # --- 2. ANALYZE & SAVE ---
#         if raw_data:
#             # FIX: Handle NoneType for direct_answer safely
#             safe_answer = direct_answer if direct_answer else "No Direct Answer"
#             print(f"  ✅ Data collected. AI Answer: {safe_answer[:50]}...")
            
#             # Save Log
#             save_raw_log(company, "Tavily", raw_data, safe_answer)
            
#             # Extract with Groq
#             result_json = analyze_with_groq(company, raw_data, safe_answer)
...
# FINAL_OUTPUT_FILE = "company_intel/Final_Company_Data_Tavily_Clean.json"
# RAW_DEBUG_FILE = "raw_search_logs_tavily.json"
...
# # ==========================================
# #  2. HELPER FUNCTIONS (CLEANING & SAVING)
# # ==========================================

# def clean_text(text):
#     """
#     Cleans raw text to remove noise, newlines, and extra spaces.
#     This saves tokens and helps Groq understand better.
#     """
#     if not text: return ""
#     # Remove newlines, tabs, and backslashes
#     text = text.replace("\n", " ").replace("\t", " ").replace("\\", "")
#     # Remove multiple spaces (e.g., "Hello    World" -> "Hello World")
...
#                 # Truncate to first 1500 chars per result to save tokens
#                 context_text += f"Source: {result['title']} | Data: {cleaned_content[:1500]} ... \n"
...
#     # Handle missing direct answer
#     if not direct_answer or direct_answer == "No Direct Answer":
#         ai_intro = "Tavily summary missing. Extract directly from Search Results below."
#     else:
#         ai_intro = f"AI Summary: {direct_answer}"

#     full_context = f"{ai_intro}\n\nSEARCH RESULTS:\n{raw_data}"
...
#                         "content": (
#                             "Extract 'Annual Revenue' (with currency) and 'Total Employee Count'. "
#                             "Ignore filler text like 'Upgrade', 'Login'. "
...
#     print(f"🚀 Starting Extraction for {len(TARGET_COMPANIES)} companies (Clean Mode)")
...
# FINAL_OUTPUT_FILE = "company_intel/Final_Company_Data_Tavily_Smart.json"
# RAW_DEBUG_FILE = "raw_search_logs_tavily.json"
...
#             for result in response.get("results", []):
#                 cleaned_content = clean_text(result['content'])
#                 context_text += f"Source: {result['title']} | Data: {cleaned_content[:1000]} ... \n"
...
# # ==========================================
# #  4. GROQ ANALYSIS (SMART TOKEN LOGIC)
# # ==========================================
...
#     # --- LOGIC CHANGED HERE ---
#     # Agar Direct Answer available hai (aur wo None nahi hai), to sirf wahi bhejo.
#     # Agar nahi hai, to Raw Data bhejo.
    
#     if direct_answer and direct_answer.strip():
#         print("    📉 Sending ONLY Direct Answer to Groq (Saving Tokens)")
#         data_to_send = f"TAVILY AI SUMMARY: {direct_answer}"
#     else:
#         print("    📈 Direct Answer missing. Sending RAW DATA to Groq.")
#         data_to_send = f"SEARCH RESULTS:\n{raw_data}"
...
#     print(f"🚀 Starting Extraction for {len(TARGET_COMPANIES)} companies (SMART MODE)")
...
#         if raw_data:
#             # Handle Display String safely
#             safe_display = direct_answer if direct_answer else "No Direct Answer"
#             print(f"  ✅ Data collected. Answer: {safe_display[:40]}...")
            
#             save_raw_log(company, "Tavily", raw_data, safe_display)
            
#             # Pass BOTH to the function, but Logic inside decides which one to use
#             result_json = analyze_with_groq(company, raw_data, direct_answer)
...
# FINAL_OUTPUT_FILE = "company_intel/Final_Company_Data_Tavily_Smart1.json"
# RAW_DEBUG_FILE = "raw_search_logs_tavily1.json"
...
#     # 🎯 SPECIFIC QUERY (No Company Info, Only Numbers)
#     query = (
#         f"Total annual revenue of {company} in {prev_year} (or latest) "
#         f"and total employee count in {current_year} (or latest). "
#         "Just the numbers."
#     )
...
#             # --- CLEANING & LIMITING RAW DATA ---
#             context_text = ""
#             for result in response.get("results", []):
#                 cleaned = clean_text(result['content'])
#                 # Har result se sirf 500 chars lo (Total 1000 chars for 2 results)
#                 context_text += f"Source: {result['title']} | Data: {cleaned[:1500]} ... \n"
...
# def analyze_with_groq(company, final_context_string):
#     """
#     Ab ye function bas "final_context_string" lega jo main() mein decide hoga.
#     """
#     max_retries = max(1, get_groq_count())
...
#     print(f"🚀 Starting Extraction for {len(TARGET_COMPANIES)} companies (SMART TOKEN SAVER)")
...
#         if raw_data: # Means search was successful
            
#             safe_answer = direct_answer if direct_answer else ""
#             bad_keywords = ["unknown", "not available", "not found", "no information"]
            
#             # --- THE LOGIC YOU ASKED FOR ---
            
#             # 1. Check if Direct Answer is Empty
#             if not safe_answer:
#                 print("    ⚠️ No Direct Answer. Using Raw Content (1000 chars).")
#                 final_context = f"SEARCH RESULTS:\n{raw_data[:1000]}"
                
#             # 2. Check if Direct Answer is Lazy (contains 'unknown')
#             elif any(k in safe_answer.lower() for k in bad_keywords):
#                 print(f"    ⚠️ Answer Vague ('{safe_answer[:20]}...'). Sending Answer + Raw Content.")
#                 # Combine both: Answer + Raw Data (Limited to 1000 chars)
#                 final_context = f"AI SUMMARY: {safe_answer}\n\nSEARCH RESULTS:\n{raw_data[:1000]}"
                
#             # 3. Direct Answer is Good
#             else:
#                 print(f"    ✅ Answer Good ('{safe_answer[:30]}...'). Using Answer Only.")
#                 final_context = f"AI SUMMARY: {safe_answer}"

#             # --- SEND TO GROQ ---
#             result_json = analyze_with_groq(company, final_context)
...
def _tavily_sync_call(company):
    ...
    # )
    query = (
        f"Current Annual Revenue or Employee Count for {company}. "
        f"Find 'Total Annual Revenue' (in {prev_year} or latest available) "
        f"and 'Total Employee Count' (in {current_year} or latest available). "
        "Just the numbers."
    )
    # query = (
    #     f"Exact 'Total Annual Revenue' and 'Employee Count' specifically for the company '{company}'. "
    #     f"Must be for '{company}' only. Exclude competitors and other companies. "
    #     f"Latest available data ({prev_year} or {current_year})."
    # )
    ...
    for attempt in range(max_retries):
        try:
            tavily_key = get_tavily_key(delay=1)
            if not tavily_key: return None, None
            client = TavilyClient(api_key=tavily_key)
            response = client.search(
                query=query, 
