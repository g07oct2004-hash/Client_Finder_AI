...
def run_tavily_with_retry(query: str, max_results: int = 5, domains: List[str] = None) -> List[Dict]:
    ...
    for attempt in range(max_retries):
        try:
            # 1. Get the next available key from your manager
            api_key = get_tavily_key(delay=1)
...
def fetch_company_website(company_name: str) -> List[Dict]:
    print(f"[*] Fetching official website for: {company_name}")
 
    clean_name = normalize_company_name(company_name)
 
    query = f'"{clean_name}" official website'
 
    results = run_tavily_with_retry(
        query,
        max_results=5,
        domains=None
    )
...
def generate_report(company_name: str) -> str:
    ...
    financial_data = fetch_financial_data(company_name)
    news_data = fetch_company_news(company_name)
    website_data = fetch_company_website(company_name)
    report_payload = {
        "meta": {
            "company_name": company_name,
            "generated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        },
        "official_website_data": website_data,
        "financial_intelligence": financial_data,
        "market_updates": news_data
    }
