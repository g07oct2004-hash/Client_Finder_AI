...
- Do NOT sound like a brochure.
- If a sentence feels like marketing copy, simplify it.
- Allow slight conversational irregularity.
- Avoid perfectly balanced sentence structure.
- Do not make all sentences similar length.
- Avoid rhythm patterns that feel AI-generated.


============================================================
SPAM FILTER FORBIDDEN LIST (NEVER USE THESE)
============================================================
These words and phrases trigger spam filters and must NEVER appear anywhere in the email:

FORBIDDEN PHRASES:
- "reach out", "touch base", "circle back"
- "game-changer", "cutting-edge", "best-in-class", "world-class"
- "I wanted to connect", "Hope this finds you well"
- "Let me know if you're interested"
- "Would love to", "Excited to share", "Thrilled to"
- "Scale your business", "Drive results", "Unlock potential"
- "Quick call", "Quick chat", "Hop on a call"
- "Free consultation", "No obligation", "Risk-free"
- "Revolutionize", "Transform", "Disrupt"
- "I came across your profile"
- "Just checking in","Here we are","team members","new"
...
REPLACEMENT LOGIC:
- Instead of "quick call" → "a short conversation"
- Instead of "reach out" → "I'm writing because" or just get to the point
- Instead of "Would love to" → "Happy to" or "Worth a conversation if..."
...
============================================================
Addressing (CRITICAL):
- Always start the email with: Hi,
- Do NOT use first name.
- Do NOT use last name.
- Do NOT use titles (Mr., Ms., etc.).
- Do NOT extract anything from {ceo_name}.
...
Solution Positioning:
- Position AnavClouds as a strategic execution partner.
- Naturally integrate ONE of the following core value lines once in the body, STRICTLY based on the Target Role:
  * If Salesforce-focused: "At AnavClouds, we help growing tech companies scale faster by building smart Salesforce CRM delivery engines—without increasing headcount."
  * If AI/Data-focused: "At AnavClouds, we help growing tech companies scale faster by building AI and data intelligence delivery engines—without increasing headcount."
  * If both Salesforce + AI: "At AnavClouds, we help growing tech companies scale faster by combining Salesforce CRM, AI, and data intelligence into a single smart delivery engine—without increasing headcount."
- Explain HOW we help (speed, efficiency) — not just WHAT we do. Do not repeat this line multiple times.

VALUE PROPOSITION SOFTENING RULE (CRITICAL):
- Break it into two natural sentences.
- Remove marketing rhythm.
- Blend it conversationally.
- Avoid standalone tone.
- Never isolate the value line.

Tone:
Concise. Confident. Human. Respectful.
Aim for 'Curious Peer' rather than 'Eager Vendor'.

SPAM MINIMIZATION RULE:
- Avoid adjectives like “accelerate”, “optimize”, “enhance”, “transform”.
- No exclamation marks.
- No urgency phrases.
- No double CTA.

PRIMARY INBOX OPTIMIZATION RULES:
- The email must NOT read like marketing or promotional content.
- Avoid structured sales formatting (no bullet points, no feature lists).
- Avoid slogans, taglines, and repeated brand positioning statements.
- Avoid phrases like "scale faster", "smart delivery engine", "cutting-edge", "optimize", "solution", "leverage", "outsource", "free up resources".
- Do not repeat the same positioning statement across emails.
- The email should feel like an internal, conversational message between professionals.

MICRO PERSONALIZATION RULE:
- Reference one specific detail from financials or market news.
- React naturally as a peer.
- Do not summarize news.
- Comment briefly.

BULLET NATURALITY RULE:
- Bullets should not sound like marketing features.
- Avoid symmetry.
- Avoid ending every bullet with a period.

DELIVERABILITY PRIORITY:
- If a sentence sounds like marketing copy, simplify it.
- If promotional, remove it.
- If urgent, soften it.
- Sound like a peer, not a vendor.

CTA NATURALITY RULE:
- CTA must feel like continuation.
- Avoid availability questions.
- Avoid calendar language.
- Avoid time pressure.
- Frame as thought, not request.

**Strictiy Follow**:ADD TO FORBIDDEN WORDS:-
The following words and patterns are strictly prohibited.
They must NOT appear anywhere in the subject line or body.
**Strictly Do not use this Banned Words in mail body or Subject**
Banned Words:
- accelerate
- certified
- Offer
...
-But use variations, synonyms, or close alternatives of banned words or Forbidden words.
If any of these appear, regenerate the email.


============================================================
STRUCTURE (STRICTLY FOLLOW)
============================================================
The email must begin exactly with:
Hi,

1. Short contextual opener (based on News / Market / Financial update).
   - MUST begin using latest News, Market, or Financial update.
   - Do NOT start with hiring reference if updates exist.

2. Clear observation about hiring volume.
   AVOID:
   - Obvious assumptions
   - Direct hiring statements
   - Overly confident internal claims

3. Value Bridge & Bullets:
STRICT BULLET FORMAT RULE (MANDATORY):
You MUST strictly follow the visual structure below. Formatting accuracy is CRITICAL.
VISUAL STRUCTURE YOU MUST OUTPUT EXACTLY:

Write a short, natural, human-sounding transition sentence ending with a colon ":"
that introduces the bullet points.
[Smart, humanized intro sentence] :
...
FORMAT RULES:
• There MUST be exactly ONE space before the colon ":"
• The colon line MUST end the sentence.
• There MUST be exactly ONE blank line after the colon line before bullets start.
• Each bullet MUST start on its own new line.
• Each bullet MUST be separated by a blank line.
• Use ONLY "*" symbol for bullets.
• NEVER place bullets immediately after the colon.

INTRO SENTENCE RULES:
• Create a smart, humanized variation of phrases like:
  "Here are some ways we can help :"
  "In similar environments, we’ve supported teams by :"
  "How we usually step in :"

• Do NOT use those exact sentences.

• Write naturally, like a real human.

BULLET CONTENT RULES:
• Provide 3–4 solution-focused bullets.
• Keep them conversational, value-driven, and natural.
• They must not sound like marketing copy.


4. Short, low-friction CTA:
   - Salesforce-focused → Mention only Salesforce.
   - AI-focused → Mention only AI / automation.
   - Both → Mention Salesforce + AI.
   - Do NOT use: "quick call", "10-15 minutes", "schedule a chat"
   - Tie CTA to context.
   - Offer, not ask.
   - Sound human.
...
-- **Before finalizing the email, silently review it and remove anything that sounds like marketing copy and spam.
Rewrite it to sound more human and conversational if needed**.
...
SUBJECT: [Simple, neutral subject line based on context. No urgency. No sales tone.]
BODY_START
[Insert Email Body Here]
BODY_END
"""
...
# Addressing (CRITICAL):
# - Never greet using ONLY the first name.
# - Examples:
#     "Kumar Sharma" → "Hi there,"
#     "John Michael Doe" → "Hi there,"
# - Never use last name, title, Mr., Ms.
